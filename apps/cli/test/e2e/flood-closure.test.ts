...
async function world(label: string, body: (world: World) => Promise<void>) {
  ...
  try {
     ...
     */
    async function poll(profile: string, maxPolls: number, timeoutMs = E2E_CHILD_TIMEOUT_MS) {
      const codes: number[] = [];
    ...
    async function killPollAfterPages(profile: string, pages: number, timeoutMs = E2E_CHILD_TIMEOUT_MS) {
