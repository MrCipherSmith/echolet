...
| Networked two-process end-to-end evidence | Planned | Implemented and passing: `test/e2e/two-process.test.ts`, 3/3 unfiltered iterations of one scenario (`t55-final-verification.md`). As of flow 003 T28, `pnpm --filter @echolet/cli test:e2e` runs `vitest run test/e2e` — now all **seven** files under `apps/cli/test/e2e/` (an eighth-in-sequence, `prekey-pool-replenishment.test.ts`, landed in flow 003 T26), **30 tests**. The script is not reliably green: a clean idle run measures green in real 885.14s, a contended run has failed with 11 `E2E child timeout`s, and one made here while other agents were concurrently editing `apps/cli` failed 4 (`t25-verification-report-r2.md`, finding R2-004). These same seven files continue to run under `pnpm test` as well |
...
**Updated:** through the CLI, a published bundle used to serve exactly one first-contact sender. `relay publish` now maintains a pool of `LIMITS.PREKEY_MIN_COUNT = 20` independently signed bundles per identity, so up to twenty distinct senders can establish a first session with the same recipient before `PREKEY_BUNDLE_UNAVAILABLE` is possible, and a re-run of `relay publish` restores availability by minting a fresh replacement for each consumed or expired slot (top-up) rather than reporting `claimable: false`. No relay change, wire change, or schema change was needed; the ninth CLI command for explicit rotation remains retired, because bundle replenishment reaches the same recovery through a command that already existed. This is a client-only change, shipped `1ed5b2a`.

**Open limitation, not closed by the pool:** the cost to attack and to recover is asymmetric. A raw claim destroys one pool member in one unauthenticated request; restoring one costs two (a resubmit plus a fresh mint-and-publish). At the shipped 120 requests/minute per-host rate limit, **one** attacking source already destroys pool members faster than the owner's own `relay publish` can restore them — up to 120 members a minute destroyed against a recovery ceiling of at most 60 a minute — and the limiter cannot separate the two because it is keyed on the peer host, not on identity. With no pool garbage collection, a sustained single-source attack at that recovery rate exhausts the one-time-prekey identifier space (`maxOneTimePreKeyId = 0xffffff`, 16 777 215 ids) in roughly 194 days, after which `relay publish` fails permanently with "One-time prekey identifiers are exhausted." See `t25-verification-report-r2.md` §3.2 and §5 (R2-006, R2-010).
