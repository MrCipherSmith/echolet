...
- **The console reports only figures a command actually returned, and explains the failures it shows** (flow 004). Two changes, both narrow and both about what the operator may believe from the screen. First, a mailbox count is now a figure some command result carried, verbatim, or the absence of one stated in words — never arithmetic. The inbox row shows what the last `poll` accepted and says that is what it is, rather than a running total the console maintained itself; the outbox row shows no figure at all and names the reason, because no command in the frozen eight-command surface reports a pending count. What it replaced was worse than optimistic: the console added one to the outbox on a *successful* send, and a delivered message leaves the pending set rather than joining it, so the number moved the opposite way from the store and nothing brought it back. Second, every failure the console shows now carries a sentence saying what happened and one saying what to do, beside the CLI's own code and exit class rather than in place of them — so `PREKEY_BUNDLE_UNAVAILABLE (exit 3)` also says that the missing prekey bundle is the other side's and that this is not a trust failure. The code and the number are what survive at the narrowest supported terminal; the words are what gets clipped.
