...
* **Пример:** `262144`
* **Default:** `262144`
* **Потолок — это протокольная константа, а не настройка деплоя.** `262144` = `LIMITS.MAX_MESSAGE_BYTES` (`packages/protocol/src/constants/limits.ts`, зеркало — `apps/relay/internal/protocol/limits.go`). Клиент выводит из этого же числа свой лимит на размер ответа `/v1/mailbox/poll` и не узнаёт размер у relay на лету. Значение **ниже** потолка допустимо: relay просто становится строже. Значение **выше** потолка relay отказывается принимать при старте (`config.Validate`) — иначе он принял бы и сохранил envelope, который любой получатель затем отверг бы целиком вместе со всем батчем, и mailbox стал бы недоставляемым молча.
* **`0` заданный явно — отказ при старте (flow 003, T22).** `ECHOLET_MAX_MESSAGE_BYTES=0` — это не «снять ограничение»: такой настройки нет, потому что потолок протокольный. Relay, стартовавший с нулём, отвечал бы `PAYLOAD_TOO_LARGE` на каждый легитимный envelope, оставаясь при этом «здоровым» по `/health`. Теперь `config.Load` отказывается стартовать и в тексте ошибки называет переменную и потолок. **Переменная, не заданная вовсе, или заданная пустой строкой (`ECHOLET_MAX_MESSAGE_BYTES=`), по-прежнему означает «не настроено» и даёт ровно потолок `262144`** — пустая строка в `.env` не мешает рестарту.
...
**Переменной больше нет и в бинарнике (flow 003, T33).** Поле
`CleanupIntervalSec` удалено из `apps/relay/internal/config/config.go`, и вызов,
который его читал (`internal/api/router/router.go`), теперь берёт период у самой
службы (`service.DefaultCleanupIntervalSeconds`, те же 60 секунд). Практические
следствия: relay больше не печатает при старте интервал, которого нет ни в одном
документе, и **переменная, оставшаяся в чьём-то `.env`, полностью инертна — она
не меняет ничего и не мешает старту** (пины:
`TestTheConfigurationSurfaceOffersNoCleanupInterval` и
`TestLoadIgnoresACleanupIntervalAnOperatorSetsAnyway` в
`apps/relay/internal/config/cleanup_interval_removed_test.go`).

Сама служба `CleanupService`, её `Stop()` и её место в последовательности
завершения relay (`18afa36`, `cmd/relay/main.go:178`) **сохранены без
изменений** — удалена конфигурация, а не служба.
