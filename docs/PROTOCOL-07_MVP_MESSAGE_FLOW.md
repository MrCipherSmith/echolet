...
|---|---|---|---|
| `PREKEY_BUNDLE_UNAVAILABLE` | 404 | `POST /v2/prekeys/claim` | Нет доступного bundle для этого получателя: весь его пул из N = 20 независимо подписанных bundle уже востребован (с flow 003 T26 `relay publish` поддерживает пул и пополняет его повторным вызовом, а не один-единственный bundle), получатель никогда не публиковался, все опубликованные bundle вышли из окна действия, либо селектор `device_id` не совпал ни с одним доступным bundle. |
