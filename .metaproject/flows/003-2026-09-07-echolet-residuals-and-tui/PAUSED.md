...
The honest arithmetic: victims per minute is `R·S/N`, so N = 20 turns 120 into 6 from one source — a 20× increase and nothing more. Holding one source below one victim per minute needs N ≥ 120, which costs the victim 7.4 s per publish and their whole request budget. And the per-unit asymmetry runs against the defender: measured, an attacker destroys a bundle in 2.0 ms while the victim mints and publishes one in 61.6 ms, so two source addresses out-drain a refilling victim and the rate limiter cannot help, being keyed on the peer host.
